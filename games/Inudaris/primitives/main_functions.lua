...
function Functions:load_player()
	...
	table.insert(players, 1, player(8,80,120,60))
	for i = 1, #players do
...
function love.wheelmoved(x, y)
	if y > 0 then
		for i = 1, #players do
			...
			if players[i].switch == 0 or players[i].switch == 1 then
				players[i].switch = players[i].switch + 1
			else
				players[i].switch = 0
			end
			for c = 1, #players[i].Equipement do
                if players[i].switch == players[i].Equipement[c].hash then
                    players[i].weapon = players[i].Equipement[c].weap
                end
            end
		end
	elseif y < 0 then
